    ...
    int vectors_x[N/B][M/B],int vectors_y[N/B][M/B])
{
  ...
  for(i=0; i<N/B; i++)           // Loop Interchange did not work 
  {
    for(j=0; j<M/B; j++)       
    {
      vectors_x[i][j] = 0;
  ...
  for(x=0; x<N/B; x++) //Loop blocking did not work 
  {
    for(y=0; y<M/B; y++) //Loop blocking did not work 
    {
      S = 4;
      while(S > 0)
      {
...
int mainexecutable(int UserSize)
{ 
  B = UserSize; 
